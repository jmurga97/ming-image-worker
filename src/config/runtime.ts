...
const runtimeSchema = z
  .object({
    ...
    QMENUT_STAGING_BUCKET_NAME: z.string().trim().min(1),
    QMENUT_MEDIA_BUCKET_NAME: z.string().trim().min(1),
    QMENUT_PUBLIC_MEDIA_BASE_URL: z.url(),
...
export function parseRuntimeConfig(env: Bindings): RuntimeConfig {
  const runtime = runtimeSchema.parse({
    ...
    QMENUT_STAGING_BUCKET_NAME: env.QMENUT_STAGING_BUCKET_NAME,
    QMENUT_MEDIA_BUCKET_NAME: env.QMENUT_MEDIA_BUCKET_NAME,
    QMENUT_PUBLIC_MEDIA_BASE_URL: env.QMENUT_PUBLIC_MEDIA_BASE_URL,
